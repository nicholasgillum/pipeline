...
source("lib/pipeline_utils.R")
source("lib/latex_utils.R")
...
Slides.raw<-read.maimages(files=targets$FileName,source="agilent", 
...
gene_names <- matrix(unique(Slides.norm$genes$GeneName),ncol=1)
...
for(i in 1:length(gene_names)) {
	output_data[i,]=divide(gene_names[i], Slides.norm, config, conditions, dye_swaps)
}
